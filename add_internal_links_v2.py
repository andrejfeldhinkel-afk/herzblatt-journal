...
def main():
    ...
    print(f"Posts with no tags (unable to auto-link): {no_tags_count}")
    print(f"Posts with tags but no related posts found: {skipped_no_related}")
    print(f"Posts updated with 'Weiterlesen' section: {updated_count}")
    print(f"Posts still without internal links: {len(blog_files) - has_links_count - updated_count}")
