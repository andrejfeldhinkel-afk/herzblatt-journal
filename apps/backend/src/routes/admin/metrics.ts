  ...
  try {
    const result = await db.execute(sql`
      SELECT
        ...
        (SELECT COUNT(*)::int FROM inbound_emails) AS inbound_emails,
        (SELECT COUNT(*)::int FROM inbound_emails WHERE status = 'unread') AS inbox_unread,
    ...
    metrics.counts = {
      ...
      inbound_emails: Number(row.inbound_emails) || 0,
      inbox_unread: Number(row.inbox_unread) || 0,
      products: Number(row.products) || 0,
      products_active: Number(row.products_active) || 0,
      products_featured: Number(row.products_featured) || 0,
